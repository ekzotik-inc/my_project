...
import { getReadOnlyParticipantAchievements } from "./achievementReadModel";
...
export async function getMiniAppStatistics(telegramUserId: string) {
  ...
  if (!period) {
    const achievements = await getReadOnlyParticipantAchievements({ participantId: participant.id, teamId: participant.teamId, periodId: null });
  ...
  const recentActions = await db
    ...
    .limit(20);
  const achievements = await getReadOnlyParticipantAchievements({ participantId: participant.id, teamId: participant.teamId, periodId: period.id });
  return {
