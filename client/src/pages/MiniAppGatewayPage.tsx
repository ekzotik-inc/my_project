...
export default function MiniAppGatewayPage() {
  ...
  return <main className="paper-grain min-h-screen bg-[#F7F4EA] px-5 pb-[calc(2rem+var(--tg-safe-bottom))] pt-[calc(3rem+var(--tg-safe-top))] text-[#163F2F]"><section className="mx-auto max-w-md"><div className="relative overflow-hidden rounded-[2rem] bg-[#163F2F] p-7 text-white shadow-[0_28px_68px_-42px_rgba(22,63,47,0.72)]"><div className="float-slow absolute -right-12 -top-12 h-36 w-36 rounded-full bg-[#C6E5AA]" /><VisibleDeedMark className="relative bg-white text-[#163F2F]" /><p className="relative mt-6 journal-kicker text-white/65">ДОБРЫЕ ДЕЛА · ВИДИМЫЙ ВКЛАД</p><h1 className="relative mt-2 text-4xl leading-[0.9]">Здравствуйте, {access.participantName || "коллега"}</h1><div className="journal-rule relative mt-5 max-w-xs opacity-55" /><p className="relative mt-4 text-sm leading-6 text-white/72">Для вас доступны личный маршрут участника и рабочее пространство {isChief ? "Chief" : "P&C"}.</p></div><div className="journal-surface -mt-5 space-y-3 rounded-b-[2rem] p-5 pt-8"><button onClick={() => { telegramImpact("light"); setLocation("/statistics"); }} className="soft-press flex w-full items-center gap-4 rounded-2xl bg-[#EDF5E9] p-4 text-left"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white text-[#2A7047]"><Route className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-extrabold">Мой маршрут</span><span className="mt-1 block text-xs leading-5 text-[#55705E]">Личный прогресс, команда, признание и помощь.</span></span><ArrowRight className="h-4 w-4 text-[#55705E]" /></button><button onClick={() => workspace.mutate({ initData })} disabled={workspace.isPending} className="soft-press flex w-full items-center gap-4 rounded-2xl bg-[#163F2F] p-4 text-left text-white disabled:opacity-65"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/12"><BriefcaseBusiness className="h-5 w-5" /></span><span className="min-w-0 flex-1"><span className="block font-extrabold">{workspace.isPending ? "Открываем…" : workspaceLabel}</span><span className="mt-1 block text-xs leading-5 text-white/68">{workspaceDescription}</span></span><ArrowRight className="h-4 w-4" /></button><p className="flex items-start gap-2 px-1 pt-2 text-[11px] leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#2A7047]" />Роль определена сервером по подписанным данным Telegram, а не по данным телефона.</p></div></section></main>;
}

export function MiniAppOnlyNotice({ error }: { error?: string }) {
  return <main className="paper-grain flex min-h-screen items-center justify-center bg-[#F7F4EA] p-5 text-[#163F2F]"><section className="journal-surface w-full max-w-sm rounded-[2rem] p-7 text-center"><VisibleDeedMark className="mx-auto bg-[#163F2F] text-white" /><p className="journal-label mt-6">ДОБРЫЕ ДЕЛА</p><h1 className="mt-2 text-3xl leading-[0.95]">Откройте приложение из Telegram</h1><div className="journal-rule mx-auto mt-5 max-w-[13rem]" /><p className="mt-4 text-sm leading-6 text-muted-foreground">Роль и доступ определяются по защищённым данным Mini App. Войдите через кнопку «Статистика» или меню бота.</p>{error ? <p className="mt-4 rounded-xl bg-[#FFF4F4] p-3 text-xs leading-5 text-[#9A363D]">{error}</p> : null}</section></main>;
}
