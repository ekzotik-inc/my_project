...
export default function StatisticsPage() {
  ...
  return <main className="min-h-screen bg-[#F8F6F1] pb-[calc(7rem+var(--tg-safe-bottom))] pl-[var(--tg-safe-left)] pr-[var(--tg-safe-right)] pt-[var(--tg-safe-top)] text-[#172033]"><div className="mx-auto max-w-xl px-4 pt-4 sm:px-6"><header className="flex items-center justify-between"><button onClick={() => { telegramImpact("light"); setLocation("/"); }} className="soft-press flex h-10 w-10 items-center justify-center rounded-2xl border border-[#E8E5DF] bg-white" aria-label="Вернуться назад"><ChevronLeft className="h-5 w-5" /></button><div className="flex items-center gap-2"><div className="text-right"><p className="journal-kicker text-[#315FF4]">ДОБРЫЕ ДЕЛА</p><p className="mt-0.5 text-xs font-bold text-[#667085]">{lastRefreshedAt ? `обновлено ${lastRefreshedAt.toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" })}` : "ваш добрый след"}</p></div><button onClick={refreshDashboard} disabled={isRefreshing} className="soft-press flex h-10 w-10 items-center justify-center rounded-2xl border border-[#E8E5DF] bg-white disabled:opacity-60" aria-label="Обновить данные"><RefreshCw className={`h-4 w-4 ${isRefreshing ? "animate-spin" : ""}`} /></button>{canFullscreen && <button onClick={() => { telegramImpact("light"); app?.requestFullscreen?.(); }} className="soft-press flex h-10 w-10 items-center justify-center rounded-2xl border border-[#E8E5DF] bg-white" aria-label="Открыть на весь экран"><Expand className="h-4 w-4" /></button>}</div></header>
  ...
  </div><nav className="kod-bottom-nav fixed bottom-[calc(0.75rem+var(--tg-safe-bottom))] left-1/2 z-20 flex w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 gap-1 rounded-[1.25rem] p-1.5">{tabItems.map(item => <button key={item.id} onClick={() => { telegramSelectionHaptic(); setTab(item.id); }} className={`soft-press flex min-w-0 flex-1 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[10px] font-bold transition-colors ${tab === item.id ? "bg-[#316CFF] text-white" : "text-[#64708E]"}`}><item.icon className="h-4 w-4" />{item.label}</button>)}</nav></main>;
}
