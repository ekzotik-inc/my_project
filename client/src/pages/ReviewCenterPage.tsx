...
import { Check, ChevronRight, CircleAlert, Clock3, FileText, Image as ImageIcon, Inbox, Search, Trophy, UsersRound, X } from "lucide-react";
...
export default function ReviewCenterPage() {
  ...
  useEffect(() => {
    if (!selectedId && visibleQueue[0]) setSelectedId(visibleQueue[0].assignmentId);
  ...
  function decide(decision: "approved" | "rejected") {
    if (!selected) return;
    const comment = decision === "rejected" ? window.prompt("Что участнику стоит дополнить? Этот комментарий придёт ему в Telegram.") : window.prompt("Комментарий для участника (необязательно)") ?? undefined;
    if (decision === "rejected" && !comment?.trim()) {
      toast.error("Для доработки нужен понятный комментарий участнику");
      return;
    }
    moderate.mutate({ assignmentId: selected.assignmentId, decision, comment: comment?.trim() || undefined });
  }
  ...
  return <div className="mx-auto max-w-[1480px] space-y-7 px-1 py-4 sm:px-5 sm:py-7">
    ...
    <section className="grid gap-5 xl:grid-cols-[minmax(330px,0.78fr)_minmax(0,1.55fr)]">
      <aside className="overflow-hidden rounded-[1.75rem] border border-black/[0.055] bg-card shadow-[0_24px_50px_-42px_rgba(32,70,46,0.38)]">
      ...
      <section className="min-h-[580px] overflow-hidden rounded-[1.75rem] border border-black/[0.055] bg-card shadow-[0_24px_50px_-42px_rgba(32,70,46,0.38)]">
        {selected ? <div className="flex h-full flex-col"><div className="border-b border-black/[0.055] bg-[linear-gradient(135deg,#F8FCF5,#FFF7F4)] p-5 sm:p-7"><div className="flex flex-col justify-between gap-5 sm:flex-row"><div><p className="text-[10px] font-extrabold tracking-[0.14em] text-[#9A363D]">РЕЗУЛЬТАТ НА ПРОВЕРКЕ</p><h2 className="mt-2 text-2xl font-extrabold tracking-[-0.04em] text-[#163F2F]">{selected.activityTitle}</h2><div className="mt-3 flex flex-wrap gap-2 text-xs font-semibold text-[#55705E]"><span className="rounded-full bg-white px-3 py-1.5 shadow-sm">{selected.participantName || "Без имени"}</span><span className="rounded-full bg-white px-3 py-1.5 shadow-sm">{selected.teamName || "Без команды"}</span><span className="rounded-full bg-[#FFF1D6] px-3 py-1.5 text-[#7A5512]">+{selected.activityPoints} баллов после approve</span></div></div><div className="text-left sm:text-right"><p className="text-xs text-muted-foreground">Отправлено</p><p className="mt-1 text-sm font-bold text-[#355544]">{compactDate(selected.submittedAt)}</p></div></div></div>
          ...
          <footer className="sticky bottom-0 flex flex-col gap-3 border-t border-black/[0.055] bg-white/95 p-4 backdrop-blur sm:flex-row sm:justify-end sm:p-5"><Button variant="outline" onClick={() => decide("rejected")} disabled={moderate.isPending} className="rounded-xl border-[#EFC7C9] text-[#A6444B] hover:bg-[#FFF4F4]"><X className="mr-2 h-4 w-4" />Попросить доработать</Button><Button onClick={() => decide("approved")} disabled={moderate.isPending} className="rounded-xl bg-[#1E6B46] hover:bg-[#155336]"><Check className="mr-2 h-4 w-4" />Подтвердить и начислить</Button></footer>
        </div> : <EmptyQueue />}
      </section>
    </section>
