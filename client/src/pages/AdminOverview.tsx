...
export default function AdminOverview() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 px-1 py-4 sm:px-5 sm:py-7">
    <section className="enter-gentle relative overflow-hidden rounded-[2rem] bg-[#163F2F] px-6 py-8 text-white shadow-[0_30px_75px_-45px_rgba(20,62,45,0.7)] sm:px-9 sm:py-10">
      <div className="float-slow absolute -right-10 -top-16 h-52 w-52 rounded-full bg-[#C6E5AA] opacity-90" /><div className="absolute -bottom-20 right-32 h-44 w-44 rotate-12 rounded-[2.5rem] bg-[#F4B7A9]" />
      <div className="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-end"><div><Badge className="border-0 bg-white/12 px-3 py-1 text-[11px] font-semibold text-white hover:bg-white/12"><Sparkles className="mr-1.5 h-3 w-3" />CHIEF CONTROL ROOM</Badge><h1 className="mt-5 max-w-xl text-4xl font-extrabold leading-[0.98] tracking-[-0.06em] sm:text-5xl">Держите программу в живом, честном ритме</h1><p className="mt-5 max-w-lg text-sm leading-6 text-white/70 sm:text-base">Здесь видны только решения, которые меняют путь команды: период, задания, проверка и своевремное признание.</p><div className="mt-7 flex flex-wrap gap-3">{reviewCount ? <Button onClick={() => setLocation("/review")} className="soft-press h-12 rounded-xl bg-[#F4B7A9] px-5 font-extrabold text-[#3C201B] hover:bg-[#FFC7BB]"><BellRing className="mr-2 h-4 w-4" />Разобрать {reviewCount} {reviewCount === 1 ? "отчёт" : "отчётов"}</Button> : <Button onClick={() => setLocation("/periods")} className="soft-press h-12 rounded-xl bg-white px-5 font-extrabold text-[#163F2F] hover:bg-[#F9FDF7]"><CalendarRange className="mr-2 h-4 w-4" />Настроить период</Button>}<Button variant="outline" onClick={() => setLocation("/activities")} className="soft-press h-12 rounded-xl border-white/20 bg-transparent px-5 font-bold text-white hover:bg-white/10 hover:text-white">Новое задание <ArrowRight className="ml-2 h-4 w-4" /></Button></div></div><ChiefSignal reviewCount={reviewCount} onReview={() => setLocation("/review")} /></div>
    </section>
...
function NextMoveRow({ move, order, onClick }: { move: NextMove; order: number; onClick: () => void }) { const colors = { rose: "bg-[#FFF0F0] text-[#A6444B]", gold: "bg-[#FFF6DF] text-[#A06F15]", sky: "bg-[#EAF5FB] text-[#36728E]", mint: "bg-[#EAF5E9] text-[#39704C]" }; const Icon = move.icon; return <button onClick={onClick} className="surface-lift flex w-full items-center gap-4 px-5 py-4 text-left sm:px-6"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${colors[move.accent]}`}><Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="block text-[10px] font-extrabold tracking-[0.12em] text-muted-foreground">ШАГ {order}</span><span className="mt-1 block font-extrabold text-[#193E2D]">{move.title}</span><span className="mt-1 block text-sm leading-5 text-muted-foreground">{move.note}</span></span><ArrowRight className="h-4 w-4 shrink-0 text-[#55705E]" /></button>; }
