...
import { ChevronLeft, ChevronRight, ImageOff, Images, LoaderCircle, Sparkles } from "lucide-react";
...
const deckGeometry = [
  ...
  { x: -4, y: -21, rotate: -4.8, scale: 0.955 },
  { x: 5, y: -43, rotate: 4.5, scale: 0.91 },
  { x: -5, y: -64, rotate: -5.8, scale: 0.865 },
  { x: 7, y: -86, rotate: 5.2, scale: 0.82 },
] as const;
...
export function ParticipantPhotoGallery({ initData }: { initData: string }) {
  ...
  const onPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    setDragX(Math.max(-104, Math.min(104, event.clientX - drag.current.startX)));
  };
